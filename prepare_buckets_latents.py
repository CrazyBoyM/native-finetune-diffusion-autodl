...
def main(args):
  ...
  if os.path.exists(args.in_json):
    ...
    print(f"no metadata / 没有元数据/元数据文件不存在: {args.in_json}")
    return
  ...
  assert len(max_reso) == 2, f"illegal resolution (not 'width,height') / 图片大小有误，请用'宽度，高度'指定: {args.max_resolution}"
